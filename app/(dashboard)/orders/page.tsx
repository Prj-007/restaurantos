  ...
  return (
    <div>
      ...
      <div className="mt-4 rounded-lg border border-zinc-200 bg-white p-4">
        <p className="text-sm font-medium text-zinc-700">New order</p>
        <div className="mt-2 flex flex-wrap items-center gap-3">
          <select value={tableId} onChange={(e) => setTableId(e.target.value)} className="rounded-md border border-zinc-300 px-2 py-1.5 text-sm">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
          {menuItems.map((mi) => (
            <div key={mi.id} className="flex items-center justify-between rounded-md border border-zinc-200 px-2 py-1.5 text-sm">
              <span>{mi.name}</span>
              <input
                type="number"
                min={0}
                ...
                className="ml-2 w-14 rounded border border-zinc-300 px-1 py-0.5 text-right"
              />
            </div>
          ))}
        </div>
            ...
            <div key={o.id} className="rounded-lg border border-zinc-200 bg-white p-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <span className="rounded-full bg-zinc-100 px-2 py-0.5 text-xs font-medium text-zinc-700">{o.status}</span>
